...
for (i in da.dists) {
  ...
  quarto_render(
    #"DashboardSummary.qmd",
    #   output_format = "all",
    #              output_dir = "output",
    #              execute_dir = "output",
    input = "DashboardSummary.qmd",

    output_file = paste0("Dashboard2025Summary", dist, ".html"),
    execute_params = dist,
  )

  file.rename(
    from = paste0("Dashboard2025Summary", dist, ".html"),
    to = here("output", paste0("Dashboard2025Summary", dist, ".html"))
  )
}
...
school.list2 <- dash2 %>%
  filter(str_detect(districtname, "Salinas Union"), rtype == "S") %>%
...
for (i in school.list) {
  ...
  file.rename(
    from = paste0("DashboardSummary", dist, ".html"),
...
indicator.bar.school.cars <- function(df, dist, schooly, indie) {
  ...
  subtit <- case_when(
    indie == "math" ~ "Points represent average Distance from Standards",
  ...
  eil_code <- school_dir %>%
    filter(str_detect(district, dist), str_detect(school, schooly)) %>%
    select(eil_code) %>%
    distinct() %>%
    unlist()

  verts <- case_when(
    indie == "math" & eil_code %in% c("ELEM", "INTMIDJR") ~ c(-95, -25, 0, 35),
  ...
  df %>%
    ...
    scale_fill_manual(values = purp.pal, drop = FALSE) +
    labs(
      title = paste0(tit, " by Student Group<br> for ", schooly),
      subtitle = subtit,
      x = "",
      y = ""
    ) +
...
indicator.bar.school.cars(
  df = dash2,
  dist = "Monterey Peninsula Unified",
  school = "La Mesa",
  indie = "ela"
)
