# Calculations to determine what is needed in future year to not be eligible for DA
...
exit.crit <- dash.mry %>%
  ...
  mutate(
    districtname = if_else(is.na(charter_flag), districtname, schoolname)
  ) %>%
  select(
    cds,
    districtname,
  ...
  mutate(
    ...
    change.thresh = case_when(
      ...
      indicator == "SUSP" &
        statuslevel == 2 &
        eil_code %in% c("ELEM", "ELEMHIGH") ~
        currstatus + 2, # Elem
      indicator == "SUSP" & statuslevel == 2 & eil_code == "INTMIDJR" ~
        currstatus + 4, # Elem
      indicator == "SUSP" & statuslevel == 2 & eil_code == "HS" ~
        currstatus + 3, # Elem

      indicator == "SUSP" & statuslevel == 2 & doc %in% c(52) ~ currstatus + 2, # Elem
      indicator == "SUSP" & statuslevel == 2 & doc %in% c("00", 54) ~
        currstatus + 2, # Unified
      ...
      indicator == "SUSP" & eil_code %in% c("ELEM", "ELEMHIGH") ~
        currstatus - .3, # Elem
  ...
  mutate(
    thresh = case_when(
      indicator == "CHRO" & statuslevel == 2 ~
        min(change.thresh, status.thresh),
      indicator == "ELPI" & statuslevel == 2 ~
        max(change.thresh, status.thresh),
      indicator == "SUSP" & statuslevel == 2 ~
        min(change.thresh, status.thresh),
    ...
    threshold = case_when(
      indicator %in% c('MATH', 'ELA') ~ as.character(thresh),
      TRUE ~ percent(thresh / 100, accuracy = 0.1)
    ),
    current = case_when(
      indicator %in% c('MATH', 'ELA') ~ as.character(currstatus),
      TRUE ~ percent(currstatus / 100, accuracy = 0.1)
    ),
    pass.count = case_when(
      # indicator %in% c('math','ela') ~ thresh,
      indicator %in% c('CHRO', 'SUSP') ~ floor(currdenom * thresh / 100),
      indicator %in% c('GRAD', 'CCI', 'ELPI') ~
        ceiling(currdenom * thresh / 100)
    ),
    comper = case_when(
      indicator %in% c('MATH', 'ELA') ~ '',
      indicator %in% c('CHRO', 'SUSP') ~ 'or less',
      indicator %in% c('GRAD', 'CCI', 'ELPI') ~ 'or more'
    ),
    pass.count.comp = case_when(
      indicator %in% c('MATH', 'ELA') ~ '',
      indicator %in% c('CHRO', 'SUSP', 'GRAD', 'CCI', 'ELPI') ~
        paste0(pass.count, " ", comper)
    ),
    adjective = case_when(
      indicator == 'CHRO' ~ 'chroncially absent',
  ...
  mutate(
    #sentence_short = glue("{studentgroup.long}  of {currdenom}"),
    sentence_full = ifelse(
      indicator %in% c("ela", "math"),
      glue(
        "{studentgroup.long} student group should have an average of {thresh} from standard or higher based on the {indicator} CAASPP exam to not be in Red."
      ),
      glue(
        "{studentgroup.long} student group should have {pass.count} {comper} students {adjective} based on the count in 2022 of {currdenom} to not be in Red."
      )
    )
  )
...
#2024
#exit.crit.ss <- "https://docs.google.com/spreadsheets/d/1SPEEZom8Kx87y3i1Yc-QEHbVgc6eeszRcWXruYwzsGg/edit?gid=0#gid=0"

#2025
exit.crit.ss <- "https://docs.google.com/spreadsheets/d/1z-pUT68MbNQVHGVxuYenKJ9Uuhu9Sqd5ZmG7AJPToT8/edit?gid=0#gid=0"
