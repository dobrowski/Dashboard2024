dash.graph <- function(df, dist, grouping = "D") {
  df %>%
    {
      if (grouping == "S") {
        filter(., str_detect(schoolname, dist))
      } else {
    ...
    geom_tile(
      aes(
        y = reorder(studentgroup.long.split, desc(studentgroup.long.split)), # Student group
    ...
    #                   na.value = "White",
    #                   direction = -1) +
    scale_fill_manual(values = color.pal, drop = FALSE) +
    ...
    labs(
      title = "<span style = 'font-size:30pt; font-family:Rockwell; color:#D55E00'>**Student Group Status**</span>",
      x = "",
      y = "",
      caption = paste0(
    ...
    guides(
      #color = guide_legend(title = "DA Eligible",   # Prettify the legends
      #                             title.position = "top",
...
indicator.bar <- function(df, dist, indie, grouping = "D", yr = yr.curr) {
  ...
  tit <- case_when(
    indie %in% c("MATH", "math") ~ "Math",
    indie %in% c("CHRO", "chronic") ~ "Chronic Absenteeism",
    indie %in% c("CCI", "cci") ~ "College Career Readiness",
    indie %in% c("GRAD", "grad") ~ "Graduation Rate",
    indie %in% c("ELPI", "elpi") ~ "English Language Progress",
    indie %in% c("SCIENCE", "science") ~ "Science",
    indie %in% c("ELA", "ela") ~ "ELA",
    indie %in% c("SUSP", "susp") ~ "Suspension",
    TRUE ~ indie
  )

  subtit <- case_when(
    indie %in% c("MATH", "math") ~ "Distance from Standard",
    indie %in% c("CHRO", "chronic") ~ "",
    indie %in% c("GRAD", "grad") ~ "",
    indie %in% c("CCI", "cci") ~ "",
    indie %in% c("ELPI", "elpi") ~
      "Percent of EL students who improve on the ELPAC",
    indie %in% c("ELA", "ela") ~ "Distance from Standard",
    indie %in% c("SCIENCE", "science") ~ "Science points",
    indie %in% c("SUSP", "susp") ~
      "Percent of students suspended at least 1 full day",
    #    TRUE ~ indie
  )
  ...
  df %>%
    {
      if (grouping == "S") {
        filter(., str_detect(schoolname, dist), rtype == "S")
      } else {
        filter(., str_detect(districtname, dist), rtype == "D")
      }
    } %>%
    ...
    mutate(
      shifty = ifelse(currstatus > 0, 1, -.05),
      labby = case_when(
        indie %in% c("MATH", "ELA", "SCIENCE", "math", "ela", "science") ~
...

    {
      if (
        indie %in%
          c("SUSP", "CCI", "CHRO", "GRAD", "susp", "cci", "chronic", "grad")
      ) {
        ylim(0.0, NA)
      }
    } +
    ...
    labs(
      title = glue(
        "<span style = 'font-size:20pt; font-family:Rockwell; color:#636363'>**{dist}<br>{tit}**</span>"
      ),
      subtitle = subtit,
      x = "",
      y = "",
...
indicator.bar2 <- function(df, dist, indie, grouping = "D", yr = yr.curr) {
  ...
  df %>%
    ...
    mutate(
      shifty = ifelse(currstatus > 0, 1, -.05),
      labby = case_when(
        indie == "MATH" ~ as.character(currstatus),
    ...
    labs(
      title = glue(
        "<span style = 'font-size:30pt; font-family:Rockwell; color:#D55E00'>**{tit}**</span>"
      ),
      subtitle = subtit,
      x = "",
      y = "",
...
candy.comp <- function(
  ...
  df.working %>%
    ggplot(aes(
      x = studentgroup.long.split,
      y = currstatus,
      group = reportingyear
    )) +
    #       ggplot(aes(x = fct_reorder(Group,DFS), y = DFS)) +
    geom_col_pattern(
      aes(fill = EstimatedColor, pattern = reportingyear, color = "black"),
      width = 0.7,
    ...
    mcoe_theme +
    {
      if (length(work.group) >= 8) {
        scale_x_discrete(guide = guide_axis(n.dodge = 2))
      }
    } + #Fixes the overlapping axis labels to make them alternate if lots of columns
    scale_fill_identity() +
    ...
    labs(
      ...
      subtitle = paste0(
        yr.curr - 1,
        " results are on the left and ",
        yr.curr,
        " are on the right for each student group"
      )
    )
...
indicator.bar.schools <- function(df, dist, indie, yr = yr.curr) {
  ...
  df %>%
    mutate(
      schoolname = if_else(is.na(schoolname), "Districtwide", schoolname)
    ) %>%
    filter(
      # str_detect(districtname, dist),
      reportingyear == yr,
...
hist.lines <- function(df = dash.hist, group, indie) {
  tit <- paste0(
    case_when(
      indie == "MATH" ~ "<img src='icons/2math.png' width='40' /> Math",
      indie == "CHRO" ~
        "<img src='icons/5chronic.png' width='40' /> Chronic Absenteeism",
      indie == "GRAD" ~
        "<img src='icons/4grad.png' width='40' /> Graduation Rate",
      indie == "ELPI" ~
        "<img src='icons/3elpi.png' width='40' /> English Languague Progress (ELPI)",
      indie == "ELA" ~ "<img src='icons/1ela.png' width='40' /> ELA",
      indie == "SUSP" ~
        "<img src='icons/6suspend.png' width='40' /> Suspension",
      TRUE ~ indie
    ),
  ...
  df %>%
    filter(studentgroup %in% group, indicator == indie) %>%
    mutate(
      status = case_when(
        indie == "MATH" ~ currstatus,
...
confetti <- function(df, dist, indie, grouping = "D") {
  ...
  df.working %>%
    ...
    geom_line(size = 2, color = "grey40") +
    geom_point(size = 5, aes(color = EstimatedColor)) +
    expand_limits(y = 0) +
    geom_text(color = "black", vjust = -1) +
    geom_blank(aes(
      x = reportingyear,
      y = currstatus * 1.2,
      label = currstatus
    ...
    labs(
      title = paste0(
        "<span style = 'font-size:20pt; font-family:Rockwell; color:#636363'>**",
        dist,
        " - ",
        tit,
        "<br>2025 Rates by Student Group**</span>"
      ),
      subtitle = paste0(
        "<span style = 'font-size:14pt; font-family:Rockwell; color:#636363'>**",
        "From ",
        yr.curr - 2,
        " through ",
        yr.curr,
  ...
  # labs(
  #   title = paste0(dist, " - ", tit, " Rates by Student Group Over Time"),
  #   subtitle = paste0(
  #     "From ",
  #     yr.curr - 2,
  #     " through ",
...
confetti.studentgroup <- function(df, dist, studentgrup, grouping = "D") {
  ...
  df.working %>%
    ...
    theme(legend.position = "none") +
    labs(
      title = paste0(dist, " - ", studentgrup, " Rates Over Time"),
