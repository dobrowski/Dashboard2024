...
This report is created to assist `r school.name` in reflecting on their student groups and how they relate to DA, CSI, and ATSI.

## Qualifying Groups

Differentiated Assistance is determined by the California Dashboard. Colors reflect the latest available status from 2023.
...
Under ESSA, schools can be identified for Comprehensive Support and Improvement (CSI) or Additional Targeted Support and Improvement (ATSI). CSI is based on a school's overall or All Students indicators, whereas ATSI is based on each student group's indicators. For more information please see: [CDE School Support](https://www.cde.ca.gov/sp/sw/t1/schoolsupport.asp). It is important to note that calculations are only based on student groups with 30 or more students.
...
atsi.words <- if ( length( atsi.who$definition) > 0 ) paste0(combine_words( atsi.who$definition), " students have been identified as meeting ATSI criteria.") else paste0(" ")
...
`r if_else (length( essa.tbl$cds) == 0, paste0(school.name," is not eligible for ATSI or CSI."), paste0(school.name," is eligible for ",essa.tbl$AssistanceStatus2023,".") )`
...
These graphs represent all of the indicators from Priority Areas that qualified the district for DA or school for ATSI. They included all DA qualifying student groups and student groups qualify for ATSI on each graph. In addition, the All Students group is included in all graphs for comparisons.
...
atsi.mry <- essa.tbl %>%
    filter( str_detect(AssistanceStatus2023, "ATSI")) %>%
    pivot_longer(cols = AA:WH, names_to = "studentgroup") %>%
    select(cds, districtname, schoolname, studentgroup, value) %>%
    filter(value >= 1) %>%
    select(-value)
...
atsi.dash <- dash.mry %>%
    filter(reportingyear == max(reportingyear) ,
           cds %in% atsi.mry$cds,
            !is.na(indicator) ) %>%
    select(cds, studentgroup, indicator, color, statuslevel) %>%
    mutate(color = case_when(indicator == "CCI" & statuslevel == 1 ~ 1, 
                              indicator == "CCI" & statuslevel != 1 ~ 0,
                              TRUE ~ color)
           ) %>%
    filter(color >= 1)
...
da.atsi.inds <- c(csi.list$indicator, atsi.dash$indicator, da.inds) %>% unique()


dash.mry.school <- dash.mry %>%
    filter(cds == cds.school)


for (i in da.atsi.inds) {
    
p <- hist.lines(df = dash.mry.school,
                group = c("ALL",da.atsi.groups),
                indie = i)
